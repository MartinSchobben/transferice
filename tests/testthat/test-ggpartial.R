test_that("partial can plot", {
  ...
  # partial regressions
  ggpartial(calc_partials(lm_fit), dplyr::select(dinodat, dplyr::any_of(sl_parms)),  PC1, t_an)
...
test_that("ranges of plots stay the same for different splits", {
  ...
  # partial regressions
  gg <- ggpartial(lm_fit, dplyr::select(dinodat, dplyr::any_of(sl_parms)),  PC1, t_an)
