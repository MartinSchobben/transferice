...
library(rsample)
dinodat_split <- initial_split(dinodat, prop = 0.75) # how to startify
...
dinodat_recipe <- recipe(fml, data = dinodat) |> 
  # step_corr(all_outcomes(), threshold = 0.8) |> # remove highly correlated environmental parameters
  # step_scale(all_outcomes()) |> # scale all responses
  step_logit(all_predictors(), offset = 0.025) |>  # make dino count log odds
  step_pca(all_predictors(), options = list(center =  TRUE)) # reduce multi-dimensionality
```
...
```{r partial}
mdl <- tuned_partials(dinodat_tuned, "Fold01", num_comp, 2) 
ggpartial(mdl, PC2, p_an)
```
