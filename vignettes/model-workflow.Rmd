...
# formula for recipes
dinodat_recipe <- recipe(x = dinodat, vars = c(ids, pms, txa), roles = rls) |> 
  # scale all outcomes
  ...
  step_mutate(env = p_an, role = "outcome")
```
...
#fml <- formula_parser(pms, paste0("PC", 1:5), type = "tidymodels")

fml <-as.formula(paste0("env~", paste0("PC", 1:5, collapse = "+")))
...
# results
outcome <- bind_cols(dinodat_test_prep_lm, dinodat_predict_lm)
```
...
long_outcome <- pivot_longer(
  outcome,
  cols = !c(sample_id, hole_id, longitude, latitude, PC1, PC2, PC3, PC4, PC5),
...
library(dials)
dinodat_recipe_tune <- recipe(x = dinodat, vars = c(ids, pms, txa), roles = rls) |> 
  # scale all outcomes
  recipes::step_normalize(recipes::all_outcomes()) |> 
  # rename taxa
  recipes::step_rename(!!!rlang::set_names(txa, paste0("taxa_", seq_along(txa))))  |> 
