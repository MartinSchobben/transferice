...
dino_loc <- dplyr::select(dino_all, site_hole = .data$station, .data$longitude, 
                          .data$latitude) |> 
  dplyr::distinct(.data$site_hole, .keep_all = TRUE) |> 
  dplyr::mutate(hole_id = dplyr::row_number(), .before = "site_hole")
...
# lose contributors
dino_smp <-  dplyr::select(dino_smp, -c(contributors, depth, weight, lycopodium, lyc_tab))
