...
# new recipe
# variables
fossil_tx <- fossil[,c(5, 6, 9:128)]
age_ma <- fossil$age_ma

vars <- role_organizer(fossil_tx, NULL, group = NULL, temporal = NULL, meta = NULL)
# taxa
txa <- vars[names(vars) == "predictor"] |> unname()

# recipe
rcp <- recipe(x = fossil_tx, vars = vars, roles = names(vars)) |> 
  # rescale predictor
  step_log(dplyr::any_of(txa), offset = 0.025) |> 
  # center predictors
  step_center(dplyr::any_of(txa)) |> 
...
# bake fossil data
fossil_prepped <- prep(rcp, training = fossil_tx) |> 
    bake(new_data = NULL)
